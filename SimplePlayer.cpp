...
status_t SimplePlayer::onDoMoreStuff() {
    ...
    for (size_t i = 0; i < mStateByTrackIndex.size(); ++i) {
        ...
        do {
            ...
            if (err == OK) {
                //printf("dequeued input buffer on track %zu\n",
                      //mStateByTrackIndex.keyAt(i));

                state->mAvailInputBufferIndices.push_back(index);
            } else {
                //printf("dequeueInputBuffer on track %zu returned %d\n",
        ...
        do {
            ...
            if (err == OK) {
                //printf("dequeued output buffer on track %zu\n",
                      //mStateByTrackIndex.keyAt(i));
                ...
                //printf("dequeueOutputBuffer on track %zu returned %d\n",
                      //mStateByTrackIndex.keyAt(i), err);
            }
        } while (err == OK
    ...
    for (;;) {
        ...
        if (err != OK) {
            ...
            //printf("enqueued input data on track %zu\n", trackIndex);
    ...
    for (size_t i = 0; i < mStateByTrackIndex.size(); ++i) {
        ...
        while (!state->mAvailOutputBufferInfos.empty()) {
            ...
            if (lateByUs > -10000ll) {
                ...
                if (lateByUs > 30000ll) {
                    //printf("track %zu buffer late by %lld us, dropping.\n",
                ...
                //printf("track %zu buffer early by %lld us.\n",
                      //mStateByTrackIndex.keyAt(i), (long long)-lateByUs);
                break;
            }
        }
